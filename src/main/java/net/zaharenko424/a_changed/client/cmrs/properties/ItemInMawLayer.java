...
public class ItemInMawLayer implements RenderLayerLike {
    ...
    public void transformToMaw(CustomModel<?> model, PoseStack poseStack){
    ...
    public <E extends LivingEntity> void render(@NotNull E livingEntity, @NotNull CustomModel<E> model, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        transformToMaw(model, poseStack);

        poseStack.mulPose(Reusable.QUATERNION.get().identity().rotateZYX(0, Mth.DEG_TO_RAD * 225, Mth.DEG_TO_RAD * 90));
    ...
    private <E extends LivingEntity> void renderArmWithSpyglass(E entity, CustomModel<E> model, ItemStack stack, PoseStack poseStack, MultiBufferSource buffer, int combinedLight) {
        poseStack.pushPose();
        ...
        renderer.renderItem(entity, stack, ItemDisplayContext.HEAD, flag, poseStack, buffer, combinedLight);
        poseStack.popPose();
    }
}